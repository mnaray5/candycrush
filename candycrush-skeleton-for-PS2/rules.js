  ...
  this.moveCandiesDown = function()
  {
    ...
    for (var col = 0; col < board.boardSize; col++)
    {
      ...
      for (var emptyRow = board.boardSize - 1; emptyRow >= 0; emptyRow--)
      {
        ...
        {
          console.log("hole found! " + emptyRow);
          break;
        }
      }
...
//Removes the current game table
this.removeTable = function(){ 
  tableElem = document.getElementById('GameTable'); //Gets the GameTable element
  tableElem.innerHTML = ""; //Sets the innerHTML to empty to remove the table
}

//Creates a game table to play
  this.makeTable = function(squares){
   
    var letter = ['a','b','c','d','e','f','g','h']; //Array pf letters for the letters on the board
    var num = ['1','2','3','4','5','6','7','8']; //Array of numbers for the numbers on the board
    var row = 'r';
  
    tableElem = document.getElementById('GameTable');

      for (var r = 0; r < 8; r++) { //Loop through the rows
        rowElem = document.createElement('tr'); //Create a row element

        for (var c = 0; c < 8; c++) { //Loop through the columns for that row
          var currentCell = letter[c]+num[r]; //Get the string interpretation of that current cell's location (ie a1, a2, a3)
          colElem = document.createElement('td'); //Create a column element

          colElem.appendChild(document.createTextNode(currentCell)); //Add the candies text number to the colElem
          colElem.style.backgroundColor = squares[r][c].candy.color; //Add the candy color
          
          if(colElem.style.backgroundColor == "yellow"){ //If the candy color is yellow, set the text color to light grey to make it visible
            colElem.style.color = "light grey";
          } else {
            colElem.style.color = "white"; //Otherwise, just use white
          }
  
          rowElem.appendChild(colElem); //Append the column element to the row elemenet
        }
        tableElem.appendChild(rowElem); //Append the row element to the table
      }
      document.getElementById("mainColumn").appendChild(tableElem); //Append the table to the main column
  }

  //Disables all arrow buttons
  this.disableAllButtons = function(){

    //Get all 4 arrow buttons and disable them
    document.getElementById("b1").disabled = true;
    document.getElementById("b4").disabled = true;               
    document.getElementById("b2").disabled = true;               
    document.getElementById("b3").disabled = true;

    //Change their background and text to default white and black respectively
    document.getElementById("b1").style.background = "Gainsboro";
    document.getElementById("b1").style.color = "Black";

    document.getElementById("b4").style.background = "Gainsboro";
    document.getElementById("b4").style.color = "Black";

    document.getElementById("b2").style.background = "Gainsboro";
    document.getElementById("b2").style.color = "Black";

    document.getElementById("b3").style.background = "Gainsboro";
    document.getElementById("b3").style.color = "Black";
  }

  //Enables the arrow buttons depending on the valid moves
  this.enableButtons = function(up,down,left,right){

    //Enable the buttons to be opposite of what was sent (ie if true, then disabled must equal false)
    document.getElementById("b1").disabled = !up;
    document.getElementById("b4").disabled = !down;               
    document.getElementById("b2").disabled = !left;               
    document.getElementById("b3").disabled = !right;

    //For whatever buttons are true and can be enabled, change their background color to Dark Slate Blue, and Text color to white
    //to indicate the user can click on them
    if(up){
      document.getElementById("b1").style.background = "DarkSlateBlue";
      document.getElementById("b1").style.color = "White";
    }
    if(down){
      document.getElementById("b4").style.background = "DarkSlateBlue";
      document.getElementById("b4").style.color = "White";
    }
    if(left){
      document.getElementById("b2").style.background = "DarkSlateBlue";
      document.getElementById("b2").style.color = "White";
    }
    if(right){
      document.getElementById("b3").style.background = "DarkSlateBlue";
      document.getElementById("b3").style.color = "White";
    }
  }

//Removes the table to create a new game
this.newGameTable = function(){
     var child = tableElem.lastElementChild; //Get the last child in the table
      while (child) { //While the last child exists
        tableElem.removeChild(child); //Remove the last child
          child = tableElem.lastElementChild; //Get the next last child
      }
  
    //Loop through the board and set each element to null, so the candy can be replaced
    for(var col = 0; col < board.boardSize; col++){
      for (var row = 0; row < board.boardSize; row++){
           board.square[row][col] = null;        
      }
    }
